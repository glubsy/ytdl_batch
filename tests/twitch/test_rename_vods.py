...
class TestIdentifyAuthorsFromDirectory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_config = {
            "streamer_name_1": {
                "directory_name": "Streamer_streams",
                "channel_id": "streamer_name_1_id",
                "author_name": ["Streamer Name"]
            },
            "streamer1_alt": {
                "directory_name": "Streamer_streams",
                "channel_id": "streamer1_alt_id",
                "author_name": ["Streamer Alt"]
            },
            "another_streamer": {
                "directory_name": "other_streams",
                "channel_id": "other_id",
                "author_name": ["Other Streamer"]
            },
            "test_streamer": {
                "directory_name": "test_dir",
                "channel_id": "test_id_1",
                "author_name": ["Test Streamer"]
            }
        }

    @patch('twitch.rename_vods.Path')
    def test_identify_authors_matching_directory(self, mock_path):
        ...
        mock_current_dir.name = "Streamer_streams"
        mock_current_dir.resolve.return_value = mock_current_dir

        authors = identify_authors_from_directory(mock_current_dir, self.mock_config)
        expected_authors = {"streamer_name_1_id", "streamer1_alt_id"}
        self.assertEqual(authors, expected_authors)
    ...
    def test_identify_authors_no_matching_directory(self, mock_path):
        ...
        mock_current_dir = Mock()
        mock_current_dir.name = "random_folder"
        mock_current_dir.resolve.return_value = mock_current_dir

        with self.assertRaises(ValueError):
            identify_authors_from_directory(mock_current_dir, self.mock_config)
...
class TestVideoMatching(unittest.TestCase):
    ...
    def test_find_no_match_too_far(self):
        """Test when no video is within time threshold (65 minutes)."""
        file_path = Path("20241024 14-29-04 [Author] title [best][].mp4")
        videos = [{
            'id': '456',
            'title': 'Stream Title',
            'created_at': '2024-10-24T12:00:00Z',  # Over 2 hours away
            'duration': '2h00m00s'
        }]
